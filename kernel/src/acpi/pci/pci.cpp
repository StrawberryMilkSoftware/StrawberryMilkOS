...
namespace PCI{
    ...
    void EnumerateFunction(uint64_t deviceAddress, uint64_t function){
        ...
        GlobalRenderer->PrintNoIncPos(GetVendorName(pciDeviceHeader->VendorID));
        GlobalRenderer->PrintNoIncPos(" / ");
        GlobalRenderer->PrintNoIncPos(GetDeviceName(pciDeviceHeader->VendorID, pciDeviceHeader->DeviceID));
        GlobalRenderer->PrintNoIncPos(" / ");
        GlobalRenderer->PrintNoIncPos(DeviceClasses[pciDeviceHeader->Class]);
        GlobalRenderer->PrintNoIncPos(" / ");
        GlobalRenderer->PrintNoIncPos(GetSubclassName(pciDeviceHeader->Class, pciDeviceHeader->Subclass));
        GlobalRenderer->PrintNoIncPos(" / ");
        GlobalRenderer->Print(GetProgIFName(pciDeviceHeader->Class, pciDeviceHeader->Subclass, pciDeviceHeader->ProgIF));
        switch (pciDeviceHeader->Class) {
            case 0x01: //mass storage cont
